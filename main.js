...
function setupTextEditor() {
    ...
    // Keep cursor-aware visualizations live as the cursor moves
    textEditor.on('cursorMove', () => {
        if (!currentVisualizer || !currentVisualizer.updateFromText) return

        // Only re-render for structures that depend on cursor position
        if (currentDS === 'linkedlist' || currentDS === 'gapbuffer' || currentDS === 'trie') {
            const text = textEditor.getText()
            currentVisualizer.updateFromText(text, textEditor)
        }
    })
...
function updateComplexity(action) {
    ...
    // Get current text stats for dynamic complexity
    ...
    // Linked List
        ...
        if (action === 'insert') {
            ...
            complexity = 'O(n)'
            operation = 'Traversal'
            description = `Linear search through ${text.length} characters`
        }
    }
    // Hash Map
        ...
        if (action === 'insert') {
            complexity = 'O(1)'
            operation = 'Insert'
            description = `Hashing word (${wordCount} unique words)`
        } else if (action === 'search') {
            complexity = 'O(1)'
            operation = 'Lookup'
    ...
    // Trie
        ...
        if (action === 'insert' || action === 'search') {
            ...
            description = `Proportional to word length (${wordCount} words in trie)`
        }
    }
    // Graph (Trie-2)
        ...
        if (action === 'insert') {
            complexity = 'O(k)'
            operation = 'Add Node'
            description = `Building graph with ${wordCount} word nodes`
        } else if (action === 'search') {
            complexity = 'O(V + E)'
            operation = 'Graph Traversal'
            description = `Traversing ${wordCount} vertices and edges`
        }
    }
    // Huffman Coding
    else if (currentDS === 'huffman') {
        if (action === 'insert') {
            const charCount = text.length
            complexity = 'O(n log n)'
            operation = 'Build Tree'
            description = `Building Huffman tree (${charCount} chars, sorting frequencies)`
        } else if (action === 'search') {
            complexity = 'O(n)'
            operation = 'Encode'
            description = 'Encoding text with Huffman codes'
        }
    }
