export class LinkedListVisualizer {
    ...
    draw() {
        ...
        // Calculate chunk index and in-node position for cursor
        const charsPerNode = 10
        const cursorChunkIndex = Math.min(Math.floor(this.cursorPosition / charsPerNode), this.characters.length - 1)
        const cursorNodeStart = cursorChunkIndex * charsPerNode
        let cursorOffsetInNode = this.cursorPosition - cursorNodeStart

        // Clamp inside the visible characters of this node (handles cursor at end-of-text)
        const cursorNodeText = this.characters[cursorChunkIndex] || ''
        cursorOffsetInNode = Math.max(0, Math.min(cursorOffsetInNode, cursorNodeText.length))

        ctx.fillStyle = '#94a3b8'
        ctx.font = '12px Inter'
        ctx.fillText(
            `${this.characters.length} nodes | Cursor in node ${cursorChunkIndex + 1}, char ${cursorOffsetInNode + 1}`,
            canvas.width / 2,
            45
        )
        ...
        visibleChunks.forEach((chunk, i) => {
            ...
            // Per-character cursor marker inside the node
            if (isCursorNode) {
                const slotWidth = nodeWidth / charsPerNode
                const cursorSlot = cursorOffsetInNode
                const clampedSlot = Math.max(0, Math.min(cursorSlot, charsPerNode))
                const cursorX = x + clampedSlot * slotWidth

                ctx.strokeStyle = '#f97316'
                ctx.lineWidth = 2
                ctx.beginPath()
                ctx.moveTo(cursorX, startY + nodeHeight + 2)
                ctx.lineTo(cursorX, startY + nodeHeight + 14)
                ctx.stroke()
            }
